...
MIGRATIONS_DIR="${SCRIPTS_DIR}/../../migration/v2_citus"
...
echo "Migrating Mirror Node Data from PostgreSQL ${OLD_DB_HOST}:${OLD_DB_PORT} to Citus ${NEW_DB_HOST}:${NEW_DB_PORT}..."
...
echo "2. Restoring flyway_schema_history to Citus ${NEW_DB_HOST}:${NEW_DB_PORT}..."
PGPASSWORD="${NEW_DB_PASSWORD}" psql -h "${NEW_DB_HOST}" -d "${NEW_DB_NAME}" -p "${NEW_DB_PORT}" -U "${NEW_DB_USER}" <flyway_schema_history.sql

echo "3. Create v2 table schemas in Citus ${NEW_DB_HOST}:${NEW_DB_PORT}..."
PGPASSWORD="${NEW_DB_PASSWORD}" psql -h "${NEW_DB_HOST}" -d "${NEW_DB_NAME}" -p "${NEW_DB_PORT}" -U "${NEW_DB_USER}" <"${MIGRATIONS_DIR}/V2.0.0__citus_init.sql"

echo "4. Creating new hypertables on Citus ${NEW_DB_HOST}:${NEW_DB_PORT}..."
PGPASSWORD=${NEW_DB_PASSWORD} psql -h "${NEW_DB_HOST}" -d "${NEW_DB_NAME}" -p "${NEW_DB_PORT}" -U "${NEW_DB_USER}" -f "${SCRIPTS_DIR}/V2.0.1__table_types.sql"

echo "5. Creating partitions on Citus ${NEW_DB_HOST}:${NEW_DB_PORT}..."
PGPASSWORD=${NEW_DB_PASSWORD} psql -h "${NEW_DB_HOST}" -d "${NEW_DB_NAME}" -p "${NEW_DB_PORT}" -U "${NEW_DB_USER}" -f "${SCRIPTS_DIR}/V2.0.2__partitioning.sql"

echo "6. Backing up tables from from PostgreSQL ${OLD_DB_HOST}:${OLD_DB_PORT}"
...
echo "7. Restoring database dump to TimescaleDB ${NEW_DB_HOST}:${NEW_DB_PORT}..."
...
echo "Migration from PostgreSQL to Citus took a total of $elapsed seconds"
exit 0
